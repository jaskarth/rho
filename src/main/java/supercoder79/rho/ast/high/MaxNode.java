...
public record MaxNode(Node left, Node right) implements Node {
    ...
    public Node lower(CodegenContext ctx) {
        return new InvokeNode(INVOKESTATIC, ClassRefs.MATH, "max", ClassRefs.methodDescriptor(ClassRefs.DOUBLE, ClassRefs.DOUBLE, ClassRefs.DOUBLE), left.lower(ctx), right.lower(ctx));
    }
